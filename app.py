...
def list_buckets():
    ...
    for idx, bucket in enumerate(buckets.items()):
        print(f'{idx + 1}. {bucket[0]}: {bucket[1]}')
    print('=============================')

def add_bucket(bucket, info):
    buckets[bucket] = info
    return f'Added {bucket} as a bucket !!'

def remove_bucket(bucket):
    if bucket in buckets.keys():
        buckets.pop(bucket)
        msg = f'Removed bucket {bucket} !'
    else:
        msg = 'Sorry, that bucket does not exist. Please try again !'
    return msg

def add_to_bucket(bucket, amt):
    if bucket in buckets:
        cap = buckets[bucket]['cap']
        balance = buckets[bucket]['balance']
        if cap:
            if amt + balance > cap:
                return 'Balance exceeds cap !'
        buckets[bucket]['balance'] += amt
        return f'Added Rs. {amt} to {bucket} !'
    else:
        return 'Sorry, that bucket does not exist. Please try again !'

def remove_from_bucket(bucket, amt):
    if bucket in buckets:
        balance = buckets[bucket]['balance']
        if balance - amt >= 0:
            buckets[bucket]['balance'] -= amt
            return f'Removed Rs. {amt} from {bucket} !'
        else:
            return 'Balance goes below zero !'
    else:
        return 'Sorry, that bucket does not exist. Please try again !'
...
def transfer(src, dest, amt):
    ...
    if src_bal - amt < 0:
        return f'{src} does not have enough balance !'
    
    if dest_cap:
        if dest_bal + amt > dest_cap:
            return f'Total money in {dest} exceeds its cap !'
